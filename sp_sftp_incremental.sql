    ...
    source_data AS (
      ...
      WITH raw_enriched AS (
        SELECT
          ...
          -- NULLIF(TRIM(`${GCP_PROJECT_ID}.in_udf.findFirstNonNullValue`(sftpData, (SELECT in_att FROM internal_attribute_mapping WHERE attribute_group = 'sr_gender' and hmz_object = 'cde_contact'))), '') AS gender,
          ...
          -- NULLIF(TRIM(`${GCP_PROJECT_ID}.in_udf.findFirstNonNullValue`(sftpData,(SELECT in_att FROM internal_attribute_mapping WHERE attribute_group = 'sr_state' and hmz_object = 'cde_contact'))), '') AS state,
          ...
          NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS birth_year_raw,

          -- CASE
          --   WHEN SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64) IS NULL
          --     OR SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64) > EXTRACT(YEAR FROM CURRENT_DATE())
          --   THEN NULL
          --   ELSE EXTRACT(YEAR FROM CURRENT_DATE()) - SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64)
          -- END AS age,
          -- CASE
          -- WHEN `${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) IS NOT NULL
          -- THEN DATE_DIFF(CURRENT_DATE(),SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE),YEAR)
          -- WHEN SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64) IS NULL
          --      OR SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64) > EXTRACT(YEAR FROM CURRENT_DATE())
          -- THEN NULL
          -- ELSE EXTRACT(YEAR FROM CURRENT_DATE()) - SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64)
          -- END AS age,
          CASE
  -- use DOB (mapBirthday) if UDF returns something and it's not a future date
  WHEN SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE) IS NOT NULL
       AND SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE) <= CURRENT_DATE()
  THEN
    -- compute years, then subtract 1 if this year's birthday hasn't occurred yet
    (
      CASE
        WHEN DATE_ADD(
               SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE),
               INTERVAL DATE_DIFF(CURRENT_DATE(), SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE), YEAR) YEAR
             ) > CURRENT_DATE()
        THEN DATE_DIFF(CURRENT_DATE(), SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE), YEAR) - 1
        ELSE DATE_DIFF(CURRENT_DATE(), SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth']) AS DATE), YEAR)
      END
    )

  -- else fallback to birth_year validation + calculation (unchanged)
  WHEN SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64) IS NULL
       OR SAFE_CAST(NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS INT64) > EXTRACT(YEAR FROM CURRENT_DATE())
  THEN NULL
...
END AS age,
          ...
          -- source_priority used as tie breaker: prefer phone-hash (1) over email-hash (2)
          CASE
      ...
      dedup_by_mdlz AS (
        SELECT
          mdlzID,
          sftpData_json AS sftpData,
          raw_ingestion_ts AS ingestion_timestamp,
          email,
          email_normalized,
          email_hashed,
          phone,
          ...
          romance,
          sports,
          biscuit_consumption_flag,
          mithai_consumption_flag,
          candies_consumption_flag,
          age,
          coupon_flag,
          ROW_NUMBER() OVER (
            PARTITION BY mdlzID
            ORDER BY raw_ingestion_ts DESC, source_priority ASC
          ) AS rn
        FROM raw_enriched
      )
    ...
    -- latest_values: same aggregation logic you had originally, untouched except it reads from our new source_data
    latest_values AS (
      SELECT
        mdlzID,
        ...
        `${GCP_PROJECT_ID}.in_udf.aggregateEmailsCombinedUpdate`(
          NULL,
          ARRAY_AGG(email ORDER BY ingestion_timestamp DESC),
          ARRAY_AGG(CAST(ingestion_timestamp AS STRING) ORDER BY ingestion_timestamp DESC)
        ) AS email,
        `${GCP_PROJECT_ID}.in_udf.aggregateEmailsCombinedUpdate`(
          NULL,
          ARRAY_AGG(email_normalized ORDER BY ingestion_timestamp DESC),
          ARRAY_AGG(CAST(ingestion_timestamp AS STRING) ORDER BY ingestion_timestamp DESC)
        ) AS email_normalized,
        `${GCP_PROJECT_ID}.in_udf.aggregateEmailsCombinedUpdate`(
          NULL,
          ARRAY_AGG(email_hashed ORDER BY ingestion_timestamp DESC),
          ARRAY_AGG(CAST(ingestion_timestamp AS STRING) ORDER BY ingestion_timestamp DESC)
        ) AS email_hashed,
        ARRAY_AGG(email ORDER BY ingestion_timestamp DESC) AS email_array,
        ARRAY_AGG(email_normalized ORDER BY ingestion_timestamp DESC) AS md_email_normalized_array,
        ARRAY_AGG(email_hashed ORDER BY ingestion_timestamp DESC) AS md_email_hashed_array,
        ARRAY_AGG(CAST(ingestion_timestamp AS STRING) ORDER BY ingestion_timestamp DESC) AS date_array,
      ...
      FROM (
        SELECT
          ...
          email_normalized,
          email_hashed,
          FIRST_VALUE(first_name IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS first_name,
          FIRST_VALUE(last_name IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_name,
          FIRST_VALUE(normalized_phone IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS normalized_phone,
          FIRST_VALUE(phone IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS phone,
          FIRST_VALUE(gaid IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS gaid,
          FIRST_VALUE(idfa IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS idfa,
          FIRST_VALUE(birthday IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS birthday,
          FIRST_VALUE(gender IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS gender,
          FIRST_VALUE(city IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS city,
          FIRST_VALUE(state IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS state,
          source,
          FIRST_VALUE(consumption_brand_preference IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS consumption_brand_preference,
          FIRST_VALUE(brand_consumption_frequency IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS brand_consumption_frequency,
          FIRST_VALUE(parent_flag IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS parent_flag,
          dessert_preference,
          FIRST_VALUE(scan_code_flag IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS scan_code_flag,
          FIRST_VALUE(pincode IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS pincode,
          FIRST_VALUE(packscan_packtype IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS packscan_packtype,
          chocolate_consumption,
          romance,
          sports,
          FIRST_VALUE(biscuit_consumption_flag IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS biscuit_consumption_flag,
          FIRST_VALUE(mithai_consumption_flag IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS mithai_consumption_flag,
          FIRST_VALUE(candies_consumption_flag IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS candies_consumption_flag,
          FIRST_VALUE(age IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS age,
          FIRST_VALUE(coupon_flag IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS coupon_flag,
          FIRST_VALUE(phone_number_normalized IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY ingestion_timestamp DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS phone_number_normalized,
          sftpData,
          phone_hashed AS phone_number_hashed
        FROM source_data
      ) lv
      GROUP BY mdlzID
    )
  ...
  (
    ...
    packscan_packtype, romance, sports, biscuit_consumption_flag, mithai_consumption_flag, candies_consumption_flag, age, coupon_flag, cde_created_at, cde_updated_at
  )
  VALUES(
    source.mdlzID,
    source.email,
    ...
    source.state, source.source, source.consumption_brand_preference, source.brand_consumption_frequency, source.parent_flag, source.dessert_preference, source.scan_code_flag, source.packscan_packtype, source.romance, source.sports, source.biscuit_consumption_flag, source.mithai_consumption_flag, 
