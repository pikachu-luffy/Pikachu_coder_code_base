...
from operators.id_service_helper import fetch_mdlz_ids
...
def fetch_last_processed_epoch_ts(bot_prefix, context):
    ...
    try:
        ...
        query_max_generation = f"SELECT MAX(landing_time) AS max_gen FROM `{PIPELINE_CONFIG['audit_table_id']}` WHERE process_name= '{dag_id}' and process_status='SUCCESS' and ya_bot_id='{bot_prefix}'"
...
# def enrich_csv_with_bot_id_and_mdlz_id(blob: str, bot_prefix: str, context) -> str:
#     try:
#         dag_id = context['dag'].dag_id
#         task_id = context['task_instance'].task_id
#         logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Handling CSV file {blob} for bot {bot_prefix}")
...
#         chunk_size = 1024 * 1024 * 25  # 25 MB
#         # chunks = pd.read_csv(pd.io.common.BytesIO(content), chunksize=chunk_size, usecols=lambda col: col in expected_schema)
#         chunks = pd.read_csv(pd.io.common.BytesIO(content), chunksize=chunk_size)
#         for chunk in chunks:
...
#                 logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Fetching mdlzIds for userId in {blob.name}")
#                 chunk['phone_norm'] = '+' + chunk['userId'].astype(str).str.lstrip('+')
#                 phones = chunk['phone_norm'].dropna().astype(str).unique().tolist()
#                 if phones:
#                     phone_to_mdlzid = asyncio.run(fetch_mdlz_ids(phones))
#                 # df['mdlzID'] = df['phone'].astype(str).map(phone_to_mdlzid)
#                     chunk['mdlzID'] = chunk['phone_norm'].map(phone_to_mdlzid)
#                 chunk.drop(columns=['phone_norm'], inplace=True, errors='ignore')
#         tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".csv")
#         chunk.to_csv(tmp.name, index=False, header=False, mode='a')
#         with open(tmp.name, 'r+') as f:
#             content = f.read()
#             f.seek(0)
#             # Get headers from the first chunk
#             headers = chunks.__next__().columns.tolist()
#             if 'mdlzID' not in headers:
#                 headers.append('mdlzID')
#             f.write(','.join(headers) + '\n' + content)
#         return tmp.name
#     except Exception as e:
#         traceback.print_exc()
#         logging.error(f"Error processing blob {blob} for bot {bot_prefix}: {e}")
#         raise e
...
def enrich_csv_with_bot_id_and_mdlz_id(blob: str, bot_prefix: str, context) -> str:
    try:
        ...
        for chunk in chunks:
            # Check and rename '_id' column to 'Id' if present
            if '_id' in chunk.columns:
                chunk.rename(columns={'_id': 'Id'}, inplace=True)
            
            # Fetch mdlzID if 'userId' column is present
            if 'userId' in chunk.columns:
                chunk['phone_norm'] = '+' + chunk['userId'].astype(str).str.lstrip('+')
                phones = chunk['phone_norm'].dropna().astype(str).unique().tolist()
                if phones:
            ...
            if not header_written:
                chunk.to_csv(tmp.name, index=False, mode='w')
                header_written = True
            else:
                chunk.to_csv(tmp.name, index=False, header=False, mode='a')
    ...
    try:
        ...
        for each_bot_prefix in bot_id_prefix_list:
            ...
            for blob_info in blob_list:
                ...
                try:
                    local_path = enrich_csv_with_bot_id_and_mdlz_id(blob, each_bot_prefix, context)
                    processed_blob_name = f'processed_files/{blob_info["name"]}'
                    processed_blob = storage.Client().bucket(PIPELINE_CONFIG['bucket_name']).blob(processed_blob_name)
                    processed_blob.upload_from_filename(local_path)
                    logging.info("Uploading processed file to GCS: %s", processed_blob_name)

                    gcs_uri = f"gs://{PIPELINE_CONFIG['bucket_name']}/{processed_blob_name}"
                    logging.info("Loading processed files from GCS: %s", gcs_uri)
        ...
        if audit_rows:
            ...
            processed_blobs = storage.Client().bucket(PIPELINE_CONFIG['bucket_name']).list_blobs(prefix=processed_folder)
            for processed_blob in processed_blobs:
                logging.info(f"Deleting processed blob: {processed_blob.name}")
                processed_blob.delete()
            logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Successfully deleted all processed blobs and updated audit records.")
        else:
...
with DAG(
    dag_id='in_ya_notification_report_load_mdlzID_dag',
