...
def enrich_csv_with_bot_id(blob: str, bot_prefix: str, context) -> str:
    try:
        dag_id = context['dag'].dag_id
        task_id = context['task_instance'].task_id
        logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Handling CSV file {blob} for bot {bot_prefix}")
        # bucket = storage.Client().bucket(PIPELINE_CONFIG['bucket_name'])
...
def download_and_prepare_csv_from_json(blob: str, bot_prefix :str, context):
            ...
            for blob_info in blob_list:
                ...
                try:
                    if blob.name.lower().endswith('.json'):
                        ...
                        local_path = download_and_prepare_csv_from_json(blob, each_bot_prefix, context)
                    else:
                        local_path = enrich_csv_with_bot_id(blob, each_bot_prefix, context)
