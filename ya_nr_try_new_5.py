...
from operators.id_service_connector import IdServiceConnector
from operators.id_service_helper import fetch_mdlz_ids_parallel
...
PIPELINE_CONFIG = {
    ...
    'lookup_table_id': JOB_CONFIG['lookup_table_id']
}
...
def log_task_failure(context):
    ...
    try:
        ...
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]:- Error in log_task_failure: {e}")
        raise e


def log_memory_usage(stage: str, context):
    dag_id = context['dag'].dag_id
    task_id = context['task_instance'].task_id
    logging.info(f"[DAG: {dag_id}] [Task: {task_id} ] Memory usage at stage '{stage}':")
    process = psutil.Process()
    mem = process.memory_info().rss / (1024 * 1024)  # Convert to MB
    logging.info(f"[MEMORY] {stage}: {mem:.2f} MB")


def get_bot_id_list(context):
    try:
        dag_id = context['dag'].dag_id
        task_id = context['task_instance'].task_id
        logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Fetching bot_id_list from lookup table")
        client = bigquery.Client(project=PIPELINE_CONFIG['project_id'])
        fetch_botid_query = f"""SELECT bot_id FROM `{PIPELINE_CONFIG['lookup_table_id']}` WHERE env_name = 'staging'"""
        query_job = client.query(fetch_botid_query)
        query_result = query_job.result()        
        bot_id_list = [row.bot_id for row in query_result]
        return bot_id_list
    except Exception as e:
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]:- Error fetching bot_id_list: {e}")
        raise e
...
def fetch_last_processed_epoch_ts(bot_prefix, context):
    ...
    try:
        ...
        query_max_generation = f"SELECT MAX(landing_time) AS max_gen FROM `{PIPELINE_CONFIG['audit_table_id']}` WHERE process_name= '{dag_id}' and process_status='SUCCESS' and source_detail like '{bot_prefix}/%'"
        ...
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]:- Error fetching last landing time of the object: {e}")
        raise e
...
def insert_audit_records(records: list[dict], context):
    ...
    try:
        ...
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]:- Error inserting audit records: {e}")
        raise e
...
def get_blobs(landing_time: int, prefix, context) -> list[dict]:
    ...
    try:
        ...
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]: Error listing new blobs: {e}")  
        raise e  



def enrich_csv_with_mdlz_id(blob, bot_prefix, context):
    ...
    try:
        ...
        logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Handling CSV file {blob.name} for bot {bot_prefix}")
        expected_schema = ["_id","campaignId","botId","runId","userId","audienceId","status","statusDetail",
        ...
        with blob.open("rb") as f:
            chunk_iter = pd.read_csv(f, chunksize=10000, dtype=str, usecols=lambda col: col in expected_schema ,low_memory=False)

            for chunk in chunk_iter:
                log_memory_usage("Before processing chunk", context)
                if "_id" in chunk.columns:
                    chunk.rename(columns={"_id": "Id"}, inplace=True)
                if "userId" in chunk.columns:
                    chunk['phone_norm'] = '+' + chunk['userId'].astype(str).str.lstrip('+')
                    phones = chunk["phone_norm"].dropna().unique().tolist()
                    if phones:
                        phone_to_mdlzid = asyncio.run(fetch_mdlz_ids_parallel(phones))
                        chunk["mdlzID"] = chunk["phone_norm"].map(phone_to_mdlzid)
                    chunk.drop(columns=["phone_norm"], inplace=True, errors='ignore')
                rows_loaded = load_dataframe_to_bigquery(chunk, context)
                total_rows_loaded += rows_loaded
                gc.collect()
                log_memory_usage("After processing chunk", context)

            if total_rows_loaded > 0:
                file_load_status = 'SUCCESS'
                load_error_message = None

        return total_rows_loaded, file_load_status, load_error_message

    except Exception as e:
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]: Error processing blob {blob.name}: {e}")
        raise e


def load_dataframe_to_bigquery(df, context):
    try:
        dag_id = context['dag'].dag_id
        task_id = context['task_instance'].task_id
        logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Loading DataFrame to BigQuery")
        bigquery_client = bigquery.Client(project=PIPELINE_CONFIG['project_id'])
        schema = [
        ...
        bigquery_load_job = bigquery_client.load_table_from_dataframe(
            df,
            PIPELINE_CONFIG['table_id'],
            job_config=job_config
        )
        bigquery_load_job.result()  # Wait for the job to complete
        output_rows = bigquery_load_job.output_rows
        logging.info(f"Loaded {bigquery_load_job.output_rows} rows into {PIPELINE_CONFIG['table_id']}")
        return output_rows
    except Exception as e:
        traceback.print_exc()
        logging.error(f"[DAG: {dag_id}] [Task: {task_id}]: Error loading DataFrame to BigQuery: {e}")
        raise e
...
def load_data_to_bigquery(**context):
    ...
    try:
        ...
        audit_rows = []
        bot_id_list = get_bot_id_list(context)
        for bot_id in bot_id_list:
            logging.info("Processing bot prefix: %s", bot_id)
            last_processed_epoch_ts = fetch_last_processed_epoch_ts(bot_id, context)
            logging.info("Last processed epoch timestamp for bot %s: %s", bot_id, last_processed_epoch_ts)
            prefix = f"{bot_id}/notifications"
            blob_list = get_blobs(last_processed_epoch_ts, prefix, context)
            if not blob_list:
                logging.info("No new files to process for bot %s. Exiting.", bot_id)
                continue
            for blob_info in blob_list:
                ...
                load_error_message = None
                try:
                    total_rows_loaded, file_load_status, load_error_message = enrich_csv_with_mdlz_id(blob, bot_id, context)
                    logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Processed file {blob_info['name']} with {total_rows_loaded} rows loaded.")
                except Exception as e:
                    traceback.print_exc()
                    total_rows_loaded = 0
                    file_load_status = 'FAILURE'
                    load_error_message = str(e)
                    logging.error(f"[DAG: {dag_id}] [Task: {task_id}]:Error processing blob {blob_info['name']}: {e}")
        ...
        if audit_rows:
            ...
            logging.info(f"For [DAG: {dag_id}] [Task: {task_id}]:No audit records to insert, skipping insert_audit_records.")
    except Exception as e:
        traceback.print_exc()
        logging.error(f"For [DAG: {dag_id}] [Task: {task_id}]:Error in load_data_to_bigquery: {e}")
        raise e
