...
def enrich_csv_with_bot_id_and_mdlz_id(blob: str, bot_prefix: str, context) -> str:
    try:
        ...
        if 'phone' in df.columns:
            logging.info(f"[DAG: {dag_id}] [Task: {task_id}] Fetching mdlzIds for phones in {blob.name}")
            df['phone_norm'] = '+' + df['phone'].astype(str).str.lstrip('+')
        ...
        for each_bot_prefix in bot_id_prefix_list:
            ...
            for blob_info in blob_list:
                ...
                try:
                    ...
                    logging.info("Processing CSV file: %s", blob.name)
