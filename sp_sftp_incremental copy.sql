...
BEGIN
  ...
  -- MERGE INTO `${GCP_PROJECT_ID}.in_harmonized.in_hmz_sftp_temp` AS target
  USING (
    WITH
    ...
    source_data AS (
      SELECT
        ...
        NULLIF(TRIM(JSON_VALUE(sftpData, '$.hs_email_bad_address')), '') AS hs_email_bad_address,
        NULLIF(TRIM(JSON_VALUE(sftpData,   '$.neverbouncevalidationresult')), '') AS neverbouncevalidationresult,
        ...
        `${GCP_PROJECT_ID}.in_udf.mapBirthday`(sftpData, ['date_of_birth', 'birth_date','birth_month','birth_year']) AS birthday,
        NULLIF(TRIM(JSON_VALUE(sftpData, '$.birth_year')), '') AS birth_year,
    ...
    latest_values AS (
      SELECT
        ...
        ARRAY_AGG(neverbouncevalidationresult ORDER BY ingestion_timestamp DESC, source_priority ASC) AS neverbouncevalidationresult_array,
        ARRAY_AGG(hs_email_bad_address ORDER BY ingestion_timestamp DESC, source_priority ASC) AS hs_email_bad_address_array,
        ...
        ANY_VALUE(birth_year) AS birth_year,
      ...
      FROM (
        SELECT
          ...
          FIRST_VALUE(birth_year IGNORE NULLS) OVER (PARTITION BY mdlzID ORDER BY lastModifiedDate DESC, createdDate  DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING  ) AS birth_year,
      ...
      -- compute age from birthday (date) or birth_year (string) and then filter
  SELECT *
  FROM (
    SELECT
      lv.*,
      CASE
        -- 1) Use birthday (exact date) when valid and not in future
        WHEN SAFE_CAST(lv.birthday AS DATE) IS NOT NULL
             AND SAFE_CAST(lv.birthday AS DATE) <= CURRENT_DATE()
        THEN
          DATE_DIFF(CURRENT_DATE(), SAFE_CAST(lv.birthday AS DATE), YEAR)
          - CASE
              WHEN FORMAT_DATE('%m%d', CURRENT_DATE()) < FORMAT_DATE('%m%d', SAFE_CAST(lv.birthday AS DATE))
              THEN 1 ELSE 0
            END

        -- 2) Fallback to birth_year when birthday missing/invalid/future
        WHEN SAFE_CAST(NULLIF(TRIM(lv.birth_year), '') AS INT64) IS NOT NULL
             AND SAFE_CAST(NULLIF(TRIM(lv.birth_year), '') AS INT64) <= EXTRACT(YEAR FROM CURRENT_DATE())
             AND SAFE_CAST(NULLIF(TRIM(lv.birth_year), '') AS INT64) >= 1900
        THEN
          EXTRACT(YEAR FROM CURRENT_DATE()) - SAFE_CAST(NULLIF(TRIM(lv.birth_year), '') AS INT64)
  ...
  (
    ...
    packscan_packtype, chocolate_consumption, romance, sports, biscuit_consumption_flag, mithai_consumption_flag, candies_consumption_flag, age,
    email_validity,coupon_flag, cde_created_at, cde_updated_at
  )
  VALUES(
    source.mdlzID,
    source.email,
    ...
    source.candies_consumption_flag, source.age,
    `${GCP_PROJECT_ID}.in_udf.mergeEmailValidity`(NULL, source.email_array, source.hs_email_bad_address_array, source.neverbouncevalidationresult_array),
     source.coupon_flag, audit_timestamp, audit_timestamp
  );
END;
