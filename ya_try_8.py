...
def handle_task_failure(context):
    """Handle task failure by logging detailed error information."""
    try:
        ...
        traceback.print_exc()
        logging.error(f"Error in handle_task_failure: {e}")
        raise e


def get_last_processed_generation():
    ...
    try:
        ...
        query_max_generation = f"SELECT MAX(landing_time) AS max_gen FROM `{PIPELINE_CONFIG['audit_table_id']}` WHERE process_name= 'in_ya_unsubscribe_report_load_dag'"
...
def get_new_gcs_blobs(since_generation: int) -> list[dict]:
...
def execute_data_load(**context):
    ...
    try:
        ...
        pipeline_load_start = datetime.now(pendulum.timezone('Asia/Kolkata'))
        record_created_at = pipeline_load_start.strftime('%Y-%m-%dT%H:%M:%S')
        last_processed_generation = get_last_processed_generation()
        new_blob_list = get_new_gcs_blobs(last_processed_generation)

        if not new_blob_list:
            logging.info("No new files to process. Exiting.")
            return

        gcs_data_hook = GCSHook()
        bigquery_client = bigquery.Client(project=PIPELINE_CONFIG['project_id'])
        india_standard_timezone = pendulum.timezone('Asia/Kolkata')
        data_ingestion_timestamp = datetime.now(india_standard_timezone)

        audit_rows = []
        
        for blob_info in new_blob_list:
            file_processing_start_time = datetime.now(pendulum.timezone('Asia/Kolkata'))
            try:
                ...
                with tempfile.NamedTemporaryFile(suffix='.csv') as temp_file:
                    ...
                    dataframe = pd.read_csv(temp_file.name, dtype=str)
                dataframe['ingestion_timestamp'] = data_ingestion_timestamp
            ...
            file_processing_end_time = datetime.now(pendulum.timezone('Asia/Kolkata'))
            ...
            audit_rows.append({
                ...
                'updated_at': datetime.now(pendulum.timezone('Asia/Kolkata')).strftime('%Y-%m-%dT%H:%M:%S')
            })

        insert_audit_records(audit_rows)
    except Exception as e:
        traceback.print_exc()
        logging.error(f"Error in execute_data_load: {e}")
        raise e
...
with DAG(
    ...
    data_load_task = PythonOperator(
        ...
        python_callable=execute_data_load,
        on_failure_callback=handle_task_failure,
    )
    ...
    # DAG task dependencies: start -> load -> end
