...
SELECT
  ...
  source_data AS (
    ...
    STRUCT(
      ...
      CASE
        WHEN SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(hsData, ['date_of_birth']) AS DATE) IS NOT NULL AND SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(hsData, ['date_of_birth']) AS DATE) <= CURRENT_DATE() THEN
          DATE_DIFF(CURRENT_DATE(), SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(hsData, ['date_of_birth']) AS DATE), YEAR)
          - CASE WHEN FORMAT_DATE('%m%d', CURRENT_DATE()) < FORMAT_DATE('%m%d', SAFE_CAST(`${GCP_PROJECT_ID}.in_udf.mapBirthday`(hsData, ['date_of_birth']) AS DATE)) THEN 1 ELSE 0 END

        -- fallback to birth_year when dob is missing/invalid/future
        WHEN SAFE_CAST(NULLIF(TRIM(JSON_VALUE(hsData, '$.birth_year')), '') AS INT64) IS NOT NULL
             AND SAFE_CAST(NULLIF(TRIM(JSON_VALUE(hsData, '$.birth_year')), '') AS INT64) <= EXTRACT(YEAR FROM CURRENT_DATE())
             AND SAFE_CAST(NULLIF(TRIM(JSON_VALUE(hsData, '$.birth_year')), '') AS INT64) >= 1900
        THEN
          EXTRACT(YEAR FROM CURRENT_DATE()) - SAFE_CAST(NULLIF(TRIM(JSON_VALUE(hsData, '$.birth_year')), '') AS INT64)
      ...
      `${GCP_PROJECT_ID}.in_udf.parseAnyTimestamp`(JSON_EXTRACT_SCALAR(hsData, '$.created_at_info')) AS magento_created_at,    
    ...
    FROM `${GCP_PROJECT_ID}.in_raw_hubspot.in_hubspot_contact`  AS t, 
    UNNEST(JSON_EXTRACT_ARRAY(t.mondelezId)) AS elem
    WHERE DATE(t.ingestTimestamp) >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
    OR NOT EXISTS (
      SELECT 1
      FROM `${GCP_PROJECT_ID}.in_raw_hubspot.in_hubspot_contact`
  ...
  latest_values AS (
    SELECT
      ...
      `${GCP_PROJECT_ID}.in_udf.mergeMultipleCommaSeparatedValues`(
      ARRAY_AGG(all_brand_platform ORDER BY lastModifiedDate DESC)
    ) AS all_brand_platform,
    `${GCP_PROJECT_ID}.in_udf.mergeMultipleCommaSeparatedValues`(
      ARRAY_AGG(brand_platform ORDER BY lastModifiedDate DESC)
    ) AS brand_platform,
    `${GCP_PROJECT_ID}.in_udf.mergeMultipleCommaSeparatedValues`(
      ARRAY_AGG(consumption_brand_preference ORDER BY lastModifiedDate DESC, createdDate DESC)
    ) AS consumption_brand_preference,
    `${GCP_PROJECT_ID}.in_udf.mergeMultipleCommaSeparatedValues`(
      ARRAY_AGG(consumption_brand_preference_for_gift ORDER BY lastModifiedDate DESC, createdDate DESC)
    ) AS consumption_brand_preference_for_gift,
    `${GCP_PROJECT_ID}.in_udf.mergeMultipleCommaSeparatedValues`(
      ARRAY_AGG(coupon_brand ORDER BY lastModifiedDate DESC, createdDate DESC)
    ) AS coupon_brand,
    ...
    FROM (
      SELECT
        ...
        consumption_brand_preference_for_gift,
        coupon_brand,
