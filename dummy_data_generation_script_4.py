...
def generate_dummy_data():
    ...
    # We'll create different combinations: phone only, email only, both, hashed_only
    ...
    for i in range(max_records):
        ...
        # Added 'hashed_only' for records with only hashed data (no plain text)
        record_type = random.choice(['phone_only', 'email_only', 'both', 'hashed_only'])
        ...
        if record_type == 'phone_only':
            # Only phone, no email, with corresponding hash
            ...
            # Only email, no phone, with corresponding hash
            ...
            # Both phone and email with their corresponding hashes
            phone = random.choice(phone_numbers)
            email = random.choice(email_addresses)
            phone_hashed = hashlib.sha256(phone.encode('utf-8')).hexdigest()
            email_hashed = hashlib.sha256(email.encode('utf-8')).hexdigest()
            ...
            # No plain phone/email, only hashed versions (vendor sends only hashes)
            phone = ""
            email = ""
            
            # Generate hashes from original data but don't store the plain text
            phone_to_hash = random.choice(phone_numbers)
            email_to_hash = random.choice(email_addresses)
            
            phone_hashed = hashlib.sha256(phone_to_hash.encode('utf-8')).hexdigest()
            email_hashed = hashlib.sha256(email_to_hash.encode('utf-8')).hexdigest()
    ...
    # Write to CSV file
    output_file = "dummy_data_with_hashes_correct_schema.csv"
    ...
    print(f"  - Phone only (with hash): {phone_only_count} records")
    print(f"  - Email only (with hash): {email_only_count} records")
    print(f"  - Both phone and email (with hashes): {both_count} records")
    print(f"  - Hashed only (no plain text): {hashed_only_count} records")
    
    print(f"\nSample data verification:")
    print(f"Sample phone number: {phone_numbers[0]} -> Hash: {hashlib.sha256(phone_numbers[0].encode('utf-8')).hexdigest()}")
    print(f"Sample email: {email_addresses[0]} -> Hash: {hashlib.sha256(email_addresses[0].encode('utf-8')).hexdigest()}")
    ...
    if sample_phone_only:
        print(f"Phone only: phone='{sample_phone_only[2]}', phone_hash='{sample_phone_only[4][:20]}...', email='', email_hash=''")
    if sample_email_only:
        print(f"Email only: email='{sample_email_only[3]}', email_hash='{sample_email_only[5][:20]}...', phone='', phone_hash=''")
    if sample_both:
        print(f"Both: phone='{sample_both[2]}', email='{sample_both[3]}', both hashes present")
    if sample_hashed_only:
        print(f"Hashed only: phone='', email='', phone_hash='{sample_hashed_only[4][:20]}...', email_hash='{sample_hashed_only[5][:20]}...'")
    
    print("\nSchema updates applied:")
    print("- Added phone_hashed and email_hashed columns using SHA256")
    print("- Added 'hashed_only' record type for vendors sending only hashed data")
    print("- When plain text is present, corresponding hash is also present")
    print("- When plain text is absent, only hash values are present")
