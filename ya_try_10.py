...
def fetch_last_processed_epoch_ts(context):
    ...
    try:
        ...
        query_max_generation = f"SELECT MAX(landing_time) AS max_gen FROM `{PIPELINE_CONFIG['audit_table_id']}` WHERE process_name= '{dag_id}'"
...
def get_blobs(landing_time: int, context) -> list[dict]:
...
def load_data_to_bigquery(**context):
    ...
    try:
        ...
        last_landing_time = fetch_last_processed_epoch_ts(context)
        blob_list = get_blobs(last_landing_time, context)

        if not blob_list:
            logging.info(f"[DAG: {dag_id}] No new files to process. Exiting.")
            return
