...
class SftpReport:
    ...
    def check_schema_mismatch_and_filter(self, df: pd.DataFrame, blob: storage.Blob) -> Tuple[List[Dict], pd.DataFrame]:
        ...
        UPDATED: Extra columns (not in expected_columns) are preserved and will be included
        in sftpData JSON instead of being sent to the error table. Only phone/email validation
        issues will be handled later in enrich_with_mdlz_ids() and can reach the error table.
        """
        error_records = []
        expected_schema = self.dag_config.get('expected_columns', [])
        ...
        raw_file_columns = df.columns.tolist()
        extra_columns = [c for c in raw_file_columns if c not in expected_schema]

        self.logger.info(f"Schema validation: Expected columns: {len(expected_schema)}, File columns: {len(raw_file_columns)}")
        if extra_columns:
            self.logger.info(f"Extra columns found (will be included in sftpData JSON): {extra_columns}")

        # New behavior: do not split rows or create error records for extra columns.
        # Let the full dataframe pass through so extra fields serialize into sftpData later.
        return error_records, df
    ...
    def get_csv_dtype_dict_from_header(self, blob: storage.Blob) -> Dict[str, type]:
        """
        Build a dtype mapping for CSV columns to prevent float conversion of phone/email fields.
        This reads only the header from GCS (no full download).
        """
        phone_column = self.dag_config.get('phone_column', 'phone')
        email_column = self.dag_config.get('email_column', 'email')
        phone_hashed_column = self.dag_config.get('phone_hashed_column', 'phone_hashed')
        ...
        self.logger.info(f"Prepared dtype specifications from header: {dtype_dict}")
        return dtype_dict
    
    def read_csv_in_chunks_with_proper_dtypes(self, blob: storage.Blob, chunksize: int) -> Any:
        """
        Stream-read CSV from GCS in chunks with dtype enforcement and cleaning for phone/email columns.
        Yields chunk-level DataFrames to keep memory usage bounded.
        """
        if chunksize <= 0:
            raise ValueError("chunksize must be > 0 for chunked reading")

        phone_column = self.dag_config.get('phone_column', 'phone')
        email_column = self.dag_config.get('email_column', 'email')

        # Build dtype dict based on header only
        dtype_dict = self.get_csv_dtype_dict_from_header(blob)

        # Stream from GCS and yield chunks
        # Note: open twice - once to read header, once for chunk iterator
        with blob.open('rb') as f:
            ...
            for df_chunk in chunk_iter:
                # Clean phone/email columns if present
                if phone_column in df_chunk.columns:
                    df_chunk[phone_column] = df_chunk[phone_column].apply(self.clean_phone_number)
                if email_column in df_chunk.columns:
                    df_chunk[email_column] = df_chunk[email_column].apply(self.clean_email_address)
                yield df_chunk
    ...
    #     """
    #     Process a CSV file from GCS. If dag_config.chunk_size is set (>0), the file is processed
    #     in chunks to bound memory usage. For small files or when chunking is disabled, falls back
    ...
    #         # If chunking is enabled via dag_config, process in streaming chunks
    #         chunk_size = int(self.dag_config.get('chunk_size', 0) or 0)

    #         if chunk_size > 0:
    #             # Streamed, chunked path
    #             self.logger.info(f"Chunked processing enabled with chunk_size={chunk_size}")

    #             # Get configurations up-front
    #             column_mappings = self.dag_config.get('column_mappings', {})
    ...
    #             # Prepare a single temporary file to append all chunk outputs
    #             tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".csv")
    #             combined_tmp_path = tmp.name
    #             first_write = True
    ...
    #             total_valid_rows = 0
    #             total_mdlzid_rows = 0
    #             total_hashed_only_rows = 0
    #             total_schema_errors = 0
    #             total_id_service_errors = 0

    #             chunk_index = 0
    #             for df in self.read_csv_in_chunks_with_proper_dtypes(blob, chunk_size):
    #                 chunk_index += 1
    #                 total_rows_processed += len(df)
    #                 self.logger.info(f"[Chunk {chunk_index}] Read {len(df)} rows")

    #                 # Selective column mapping based on what's actually received
    #                 if column_mappings:
    #                     received_mapped_columns, missing_mapped_columns, received_column_mapping = self.identify_received_columns(df)
    #                     if received_column_mapping:
    #                         df.rename(columns=received_column_mapping, inplace=True)
    #                         self.logger.info(f"[Chunk {chunk_index}] Applied selective column mappings: {received_column_mapping}")
    #                     else:
    #                         self.logger.warning(f"[Chunk {chunk_index}] No columns from column_mappings found in the chunk")

    #                     # Add missing columns as NULL (do not overwrite existing)
    #                     for missing_col in missing_mapped_columns:
    #                         if missing_col not in df.columns:
    #                             df[missing_col] = None
    #                             self.logger.debug(f"[Chunk {chunk_index}] Added missing column '{missing_col}' as NULL")
    #                         else:
    #                             self.logger.debug(f"[Chunk {chunk_index}] Skipped adding missing column '{missing_col}' (already exists)")

    #                 self.logger.info(f"[Chunk {chunk_index}] Columns after selective mapping: {df.columns.to_list()}")

    #                 # Schema check per chunk
    #                 schema_error_records, valid_df = self.check_schema_mismatch_and_filter(df, blob)
    #                 if schema_error_records:
    #                     total_schema_errors += len(schema_error_records)
    #                     self.logger.warning(f"[Chunk {chunk_index}] Found {len(schema_error_records)} schema mismatch errors")
    #                     self.load_error_records(schema_error_records, blob)

    #                 if valid_df.empty:
    #                     self.logger.warning(f"[Chunk {chunk_index}] No valid rows after schema validation")
    #                     # Free chunk memory
    #                     del df
    #                     del valid_df
    #                     gc.collect()
    ...
    #                 self.logger.info(f"[Chunk {chunk_index}] Proceeding with {len(valid_df)} valid rows")

    #                 # Enrichment with mdlzID if columns exist; else prepare defaults
    ...
    #                     self.logger.info(f"[Chunk {chunk_index}] Before error removal: {successful_before_removal} with mdlzID, {hashed_before_removal} with hashed identifiers")

    #                     if id_service_errors:
    #                         total_id_service_errors += len(id_service_errors)
    #                         self.logger.warning(f"[Chunk {chunk_index}] Failed to fetch mdlzIDs for {len(id_service_errors)} records")
    #                         self.load_error_records(id_service_errors, blob)
    #                         self.logger.info(f"[Chunk {chunk_index}] Error records processed; records with hashed identifiers remain with mdlzID=None")
    #                 else:
    ...
    #                     self.logger.warning(f"[Chunk {chunk_index}] No rows remaining after ID service processing")
    #                     # Free chunk memory
    #                     del df
    #                     del valid_df
    #                     del enriched_df
    ...
    #                 # Update counters for final composition
    #                 total_valid_rows += len(enriched_df)
    #                 total_mdlzid_rows += len(enriched_df[enriched_df['mdlzID'].notna()])
    #                 total_hashed_only_rows += len(enriched_df[enriched_df['mdlzID'].isna()])

    #                 # Ensure final columns and order, fill missing columns as None
    #                 if final_columns:
    #                     for col in final_columns:
    #                         if col not in enriched_df.columns:
    #                             enriched_df[col] = None
    #                     enriched_df = enriched_df[final_columns]
    #                     self.logger.info(f"[Chunk {chunk_index}] Reordered columns to match final schema")

    #                 # Transform and append to combined temp file
    #                 transformed_df = self.transform_to_json_format(enriched_df, blob)
    #                 # Append mode after first write; write header only once
    ...
    #                 )
    #                 if first_write:
    #                     self.logger.info(f"[Chunk {chunk_index}] Initialized combined temp file: {combined_tmp_path}")
    #                     first_write = False
    #                 else:
    #                     self.logger.info(f"[Chunk {chunk_index}] Appended to combined temp file: {combined_tmp_path}")
    ...
    #             if first_write:
    #                 # No data was written (no valid rows across all chunks)
    #                 self.logger.warning("No valid rows to process across all chunks")
    #                 return None

    #             self.logger.info(f"Final main table composition across all chunks: {total_valid_rows} total records")
    #             self.logger.info(f"  - {total_mdlzid_rows} records with mdlzID")
    #             self.logger.info(f"  - {total_hashed_only_rows} records with hashed identifiers only (mdlzID=None)")
    #             if total_schema_errors:
    #                 self.logger.info(f"Total schema mismatch errors across chunks: {total_schema_errors}")
    #             if total_id_service_errors:
    #                 self.logger.info(f"Total ID service errors across chunks: {total_id_service_errors}")

    #             self.logger.info(f"Processed CSV (chunked) saved to temporary file: {combined_tmp_path}")
    #             return combined_tmp_path

    #         # Fallback to existing behavior for small files or when chunking is disabled
    #         content = blob.download_as_bytes()
    #         df = self.read_csv_with_proper_dtypes(content)

    #         # Existing logic preserved from here on
    #         column_mappings = self.dag_config.get('column_mappings', {})
    #         if column_mappings:
    #             received_mapped_columns, missing_mapped_columns, received_column_mapping = self.identify_received_columns(df)
    #             if received_column_mapping:
        ...
        try:
            ...
            if chunk_size > 0:
                ...
                for df in self.read_csv_in_chunks_with_proper_dtypes(blob, chunk_size):
                    ...
                    # IMPORTANT: Preserve a copy with all columns for sftpData JSON
                    enriched_df_for_json = enriched_df.copy()
                    ...
                    # Transform and append to combined temp file
                    # Build JSON from the full set of columns to include extra/unmapped fields
                    transformed_df = self.transform_to_json_format(enriched_df_for_json, blob)
                    # Append mode after first write; write header only once
                    transformed_df.to_csv(
                        combined_tmp_path,
                        index=False,
