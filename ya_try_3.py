...
def process_files_fn(**context):
    ...
    print(f"BigQuery Client initialized : {bq_client}")
    ...
    for item in candidates:
        ...
        logger.info("Downloading CSV %s", uri)
        ...
        # Ensure non-ingest columns are strings
        for col in ['phone','user_request','created_at','updated_at']:
            if col in df.columns:
                df[col] = df[col].astype(str)

                # Add ingestion_timestamp column with IST now
        ...
        logger.info("Loading DataFrame of %d rows into %s", len(df), TABLE_ID)
        start = time.time()
        try:
            ...
            logger.info("Loaded %s in %.2f seconds", blob_path, elapsed)
        except Exception as e:
            logger.error("Failed to load %s: %s", blob_path, str(e))
            raise
