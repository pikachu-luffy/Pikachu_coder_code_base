...
def enrich_csv_with_bot_id_and_mdlz_id(blob: str, bot_prefix: str, context) -> str:
    try:
        ...
        if 'userId' in df.columns:
            ...
            df['phone_norm'] = '+' + df['userId'].astype(str).str.lstrip('+')
            phones = df['phone_norm'].dropna().astype(str).unique().tolist()
            if phones:
                phone_to_mdlzid = asyncio.run(fetch_mdlz_ids(phones))
                # df['mdlzID'] = df['phone'].astype(str).map(phone_to_mdlzid)
                df['mdlzID'] = df['phone_norm'].map(phone_to_mdlzid)
            df.drop(columns=['phone_norm'], inplace=True, errors='ignore')
